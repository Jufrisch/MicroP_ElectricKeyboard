/*
 *Lab 8, Section 3
 *Name: Justin Frisch
 *Class #: 11476
 *PI Name: Christopher Taleno
 * Purpose: Using the DMA, DAC, and TCC to do things with less CPU involvement 
 */  

//includes
#include <avr/io.h>
#include <avr/interrupt.h>
extern void clock_init(void);

//Prototypes
void dac_init(void);
void tcc0_init(void);
void dma_init(void);

//global variables
uint8_t ii = 0;
uint16_t arr[] = {0x800,0x832,0x864,0x896,0x8c8,0x8fa,0x92c,0x95e,
0x98f,0x9c0,0x9f1,0xa22,0xa52,0xa82,0xab1,0xae0,
0xb0f,0xb3d,0xb6b,0xb98,0xbc5,0xbf1,0xc1c,0xc47,
0xc71,0xc9a,0xcc3,0xceb,0xd12,0xd39,0xd5f,0xd83,
0xda7,0xdca,0xded,0xe0e,0xe2e,0xe4e,0xe6c,0xe8a,
0xea6,0xec1,0xedc,0xef5,0xf0d,0xf24,0xf3a,0xf4f,
0xf63,0xf76,0xf87,0xf98,0xfa7,0xfb5,0xfc2,0xfcd,
0xfd8,0xfe1,0xfe9,0xff0,0xff5,0xff9,0xffd,0xffe,
0xfff,0xffe,0xffd,0xff9,0xff5,0xff0,0xfe9,0xfe1,
0xfd8,0xfcd,0xfc2,0xfb5,0xfa7,0xf98,0xf87,0xf76,
0xf63,0xf4f,0xf3a,0xf24,0xf0d,0xef5,0xedc,0xec1,
0xea6,0xe8a,0xe6c,0xe4e,0xe2e,0xe0e,0xded,0xdca,
0xda7,0xd83,0xd5f,0xd39,0xd12,0xceb,0xcc3,0xc9a,
0xc71,0xc47,0xc1c,0xbf1,0xbc5,0xb98,0xb6b,0xb3d,
0xb0f,0xae0,0xab1,0xa82,0xa52,0xa22,0x9f1,0x9c0,
0x98f,0x95e,0x92c,0x8fa,0x8c8,0x896,0x864,0x832,
0x800,0x7cd,0x79b,0x769,0x737,0x705,0x6d3,0x6a1,
0x670,0x63f,0x60e,0x5dd,0x5ad,0x57d,0x54e,0x51f,
0x4f0,0x4c2,0x494,0x467,0x43a,0x40e,0x3e3,0x3b8,
0x38e,0x365,0x33c,0x314,0x2ed,0x2c6,0x2a0,0x27c,
0x258,0x235,0x212,0x1f1,0x1d1,0x1b1,0x193,0x175,
0x159,0x13e,0x123,0x10a,0xf2,0xdb,0xc5,0xb0,
0x9c,0x89,0x78,0x67,0x58,0x4a,0x3d,0x32,
0x27,0x1e,0x16,0xf,0xa,0x6,0x2,0x1,
0x0,0x1,0x2,0x6,0xa,0xf,0x16,0x1e,
0x27,0x32,0x3d,0x4a,0x58,0x67,0x78,0x89,
0x9c,0xb0,0xc5,0xdb,0xf2,0x10a,0x123,0x13e,
0x159,0x175,0x193,0x1b1,0x1d1,0x1f1,0x212,0x235,
0x258,0x27c,0x2a0,0x2c6,0x2ed,0x314,0x33c,0x365,
0x38e,0x3b8,0x3e3,0x40e,0x43a,0x467,0x494,0x4c2,
0x4f0,0x51f,0x54e,0x57d,0x5ad,0x5dd,0x60e,0x63f,
0x670,0x6a1,0x6d3,0x705,0x737,0x769,0x79b,0x7cd};

//Main program
int main(void){
	clock_init();
	//PMIC.CTRL = PMIC_LOLVLEN_bm;
	//sei();
	
	dac_init();
	dma_init();
	tcc0_init();
	
	while (1){


	}
	
}

/* *************************************************************
* Name:     dac_init
* Purpose:  Initialize the DAC for CH0
* Inputs:
* Output:
**************************************************************/
void dac_init(void){
	
	//Set Channel operation as Channel 0 and enable event trigger on CH0
	DACA_CTRLB = DAC_CHSEL_SINGLE_gc | DAC_CH0TRIG_bm;
	//Configure the event channel
	DACA_EVCTRL = DAC_EVSEL0_bp;
	//Reference voltage
	DACA_CTRLC = DAC_REFSEL_AREFB_gc;
	//Enable channel 0 on DAC (Pin2)
	DACA_CTRLA = DAC_CH0EN_bm | DAC_ENABLE_bm;
	
	EVSYS_CH0CTRL = 0;
}
/* *************************************************************
* Name:     ISR TCC0_OVF_vect
* Purpose:  Used to initiate the DMA transfer at the proper time
* Inputs:
* Output:
**************************************************************/
ISR(TCC0_OVF_vect){
	DMA.CH0.CTRLA |= DMA_CH_ENABLE_bm;
	//Trigger is DRE flag
	DMA.CH0.CTRLA |= DMA_CH_TRFREQ_bm;
	//Clear tcc0 flag
	TCC0_INTFLAGS = TC0_OVFIF_bm;
	
}

/* *************************************************************
* Name:     tcc0_init
* Purpose:  Initialize the TCC0 for Interrupt to initiate the DMA 
* Inputs:
* Output:
**************************************************************/
void tcc0_init(void){
	//highest frequency I get is 1195Hz
	uint16_t PER = (35);
	TCC0.PER = PER;
	
	TCC0.INTCTRLA = TC_OVFINTLVL_LO_gc;
	TCC0.CTRLA = TC_CLKSEL_DIV1_gc;
	//EVSYS_CH0MUX = EVSYS_CHMUX_TCC0_OVF_gc;
}
/* *************************************************************
* Name:     dma_init
* Purpose:  Initialize the DMA system to burst 2 bytes of data to the DACA
* Inputs:
* Output:
**************************************************************/
void dma_init(void){
	//reset DMA peripheral
	DMA.CTRL |= DMA_RESET_bm;
	
	// Single burst, burst is 2 bytes
	DMA.CH0.CTRLA = DMA_CH_SINGLE_bm | DMA_CH_BURSTLEN_2BYTE_gc | DMA_CH_REPEAT_bm;
	
	//DMA ADDRESS using burst
	DMA.CH0.ADDRCTRL = DMA_CH_SRCRELOAD_TRANSACTION_gc | DMA_CH_SRCDIR_INC_gc |
	DMA_CH_DESTRELOAD_BURST_gc | DMA_CH_DESTDIR_INC_gc;
	
	//DMA Source Trigger
	DMA.CH0.TRIGSRC = DMA_CH_TRIGSRC_DACA_CH0_gc;
	
	//Size of source block
	DMA.CH0.TRFCNT = (uint16_t)(sizeof(arr));
	//Source address
	DMA.CH0.SRCADDR0 = (uint16_t)((uintptr_t)&arr[0]);
	DMA.CH0.SRCADDR1 = (uint16_t)(((uintptr_t)&arr[0])>>8);
	DMA.CH0.SRCADDR2 = 0; //(uint8_t)(((uint32_t)((uintptr_t)arr))>>16);
	
	//Destination address
	DMA.CH0.DESTADDR0 = (uint16_t)((uintptr_t)&DACA_CH0DATAL);
	DMA.CH0.DESTADDR1 = (uint16_t)(((uintptr_t)&DACA_CH0DATAH)>>8);
	DMA.CH0.DESTADDR2 = 0; //(uint8_t)(((uint32_t)((uintptr_t)&DACA_CH0DATA))>>16);
	
	//enable channel 0
	DMA.CH0.CTRLA |= DMA_CH_ENABLE_bm;
	
	//enable DMA
	DMA.CTRL |= DMA_ENABLE_bm;
}