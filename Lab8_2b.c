 ...
 * Purpose: Using the DAC to use events instead of interrupts
 */  
//includes
#include <avr/io.h>
...
void dac_init(void){
	...
	//Set Channel operation as Channel 0 and enable event trigger on CH0
	DACA_CTRLB = DAC_CHSEL_SINGLE_gc | DAC_CH0TRIG_bm;
	//Configure the event channel
	DACA_EVCTRL = DAC_EVSEL0_bp;
	//Reference voltage
...
/* *************************************************************
* Name:     tcc0_init
* Purpose:  Initialize the TCC0 for 1760Hz
* Inputs:
* Output:
**************************************************************/
void tcc0_init(void){
	//32,000,000/(2*1760*256)=35
	uint16_t PER = (72);
	TCC0_PER = PER;
